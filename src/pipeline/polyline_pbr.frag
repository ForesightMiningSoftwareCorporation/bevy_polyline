...
layout(set = 3, binding = 0) uniform PolylinePbrMaterial_base_color {
    vec4 base_color;
};

#ifdef POLYLINEPBRMATERIAL_BASE_COLOR_TEXTURE
layout(set = 3, binding = 1) uniform texture2D PolylinePbrMaterial_base_color_texture;
layout(set = 3,
       binding = 2) uniform sampler PolylinePbrMaterial_base_color_texture_sampler;
#endif

#ifndef POLYLINEPBRMATERIAL_UNLIT

layout(set = 3, binding = 3) uniform PolylinePbrMaterial_roughness {
    float perceptual_roughness;
};

layout(set = 3, binding = 4) uniform PolylinePbrMaterial_metallic {
    float metallic;
};

#    ifdef POLYLINEPBRMATERIAL_METALLIC_ROUGHNESS_TEXTURE
layout(set = 3, binding = 5) uniform texture2D PolylinePbrMaterial_metallic_roughness_texture;
layout(set = 3,
       binding = 6) uniform sampler PolylinePbrMaterial_metallic_roughness_texture_sampler;
#    endif

layout(set = 3, binding = 7) uniform PolylinePbrMaterial_reflectance {
    float reflectance;
};

#    ifdef POLYLINEPBRMATERIAL_NORMAL_MAP
layout(set = 3, binding = 8) uniform texture2D PolylinePbrMaterial_normal_map;
layout(set = 3,
       binding = 9) uniform sampler PolylinePbrMaterial_normal_map_sampler;
#    endif

#    if defined(POLYLINEPBRMATERIAL_OCCLUSION_TEXTURE)
layout(set = 3, binding = 10) uniform texture2D PolylinePbrMaterial_occlusion_texture;
layout(set = 3,
       binding = 11) uniform sampler PolylinePbrMaterial_occlusion_texture_sampler;
#    endif

layout(set = 3, binding = 12) uniform PolylinePbrMaterial_emissive {
    vec4 emissive;
};

#    if defined(POLYLINEPBRMATERIAL_EMISSIVE_TEXTURE)
layout(set = 3, binding = 13) uniform texture2D PolylinePbrMaterial_emissive_texture;
layout(set = 3,
       binding = 14) uniform sampler PolylinePbrMaterial_emissive_texture_sampler;
#    endif
